...
class DNACell(tf.nn.rnn_cell.RNNCell):
    def __init__(self, inputs, hparams, reuse=None):
        ...
        state_size = {'time': tf.TensorShape([]),
                      ...
                      'last_images': [tf.TensorShape(image_shape)] * (self.hparams.last_frames + self.hparams.jump_frames),
        ...
        if 'pix_distribs' in inputs:
            ...
            state_size['last_pix_distribs'] = [tf.TensorShape([height, width, num_motions])] * (self.hparams.last_frames + self.hparams.jump_frames)
        if 'states' in inputs:
    ...
    def zero_state(self, batch_size, dtype):
        ...
        init_state['last_images'] = [self.inputs['images'][0]] * (self.hparams.last_frames + self.hparams.jump_frames)
        if 'pix_distribs' in self.inputs:
            init_state['last_pix_distribs'] = [self.inputs['pix_distribs'][0]] * (self.hparams.last_frames + self.hparams.jump_frames)
        return init_state
    ...
    def call(self, inputs, states):
        ...
        if self.hparams.last_frames and self.hparams.num_transformed_images:
            if self.hparams.transformation == 'flow':
                ...
                with tf.variable_scope('flows'):
                    out_channels = 2 * self.hparams.jump_frames * self.hparams.last_frames * self.hparams.num_transformed_images
                    flows = conv2d(h_flow, out_channels, kernel_size=(3, 3), strides=(1, 1))
                    flows = tf.reshape(flows, [batch_size, height, width, 2, self.hparams.jump_frames, self.hparams.last_frames * self.hparams.num_transformed_images])
                    t_mod = (t + 1 - self.hparams.context_frames + self.hparams.jump_frames) % self.hparams.jump_frames
                    flows = flows[:, :, :, :, t_mod, :]
            else:
                if self.hparams.jump_frames != 1:
                    raise NotImplementedError
        ...
        with tf.name_scope('transformed_images'):
            ...
            if self.hparams.last_frames and self.hparams.num_transformed_images:
                t_mod = (t + 1 - self.hparams.context_frames + self.hparams.jump_frames) % self.hparams.jump_frames
                last_images_mod = tf.stack(last_images, axis=0)
                last_images_mod = last_images_mod[:tf.shape(last_images)[0] - t_mod][-self.hparams.last_frames:]
                last_images_mod = tf.unstack(last_images_mod, axis=0, num=self.hparams.last_frames)
                if self.hparams.transformation == 'flow':
                    transformed_images.extend(apply_flows(last_images_mod, flows))
                else:
                    transformed_images.extend(apply_kernels(last_images_mod, kernels, self.hparams.dilation_rate))
        ...
        if 'pix_distribs' in inputs:
            with tf.name_scope('transformed_pix_distribs'):
                ...
                if self.hparams.last_frames and self.hparams.num_transformed_images:
                    last_pix_distribs_mod = tf.stack(last_pix_distribs, axis=0)
                    last_pix_distribs_mod = last_pix_distribs_mod[:tf.shape(last_pix_distribs)[0] - t_mod][-self.hparams.last_frames:]
                    last_pix_distribs_mod = tf.unstack(last_pix_distribs_mod, axis=0, num=self.hparams.last_frames)
                    if self.hparams.transformation == 'flow':
                        transformed_pix_distribs.extend(apply_flows(last_pix_distribs_mod, flows))
                    else:
                        transformed_pix_distribs.extend(apply_kernels(last_pix_distribs_mod, kernels, self.hparams.dilation_rate))
                if self.hparams.prev_image_background:
...
class SAVPJumpVideoPredictionModel(VideoPredictionModel):
    def __init__(self, *args, **kwargs):
        super(SAVPJumpVideoPredictionModel, self).__init__(
    ...
    def get_default_hparams_dict(self):
        default_hparams = super(SAVPJumpVideoPredictionModel, self).get_default_hparams_dict()
        hparams = dict(
            l1_weight=1.0,
            l2_weight=0.0,
            d_net='legacy',
    ...
    def parse_hparams(self, hparams_dict, hparams):
        hparams = super(SAVPJumpVideoPredictionModel, self).parse_hparams(hparams_dict, hparams)
        if self.mode == 'test':
